function [temple] = get_temple(img)
% gets IPD
...
avgheight = round(((peak1(2)+boxes1(1,2)) + (peak2(2)+boxes2(1,2)))/2);
assignin('base','avgheight',avgheight);

% facial edge detection
img2 = rgb2gray(img1);
bw = edge(img2,'Sobel',0.05,'nothinning');

count = 0;
xloc = peak1(1)+boxes1(1,1);
while count<=4
	disp(xloc);
	xloc = xloc - 1;
	if bw(avgheight,xloc)==1
		count = count+1;
	end
end
xloc1 = xloc;
